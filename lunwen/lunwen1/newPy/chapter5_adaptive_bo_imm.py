...
# --- 第五章核心：全矩阵自适应参数 ---
WINDOW_SIZE = 20
UPDATE_INTERVAL = 3  # 更新频率
BO_INIT_POINTS = 5  # 探索点数
BO_ITERATIONS = 10  # 迭代次数
...
def main_ultimate():
    ...
    p11_history = np.zeros(num_steps)  # 记录核心参数变化
    state_snapshots = {}
    ...
    ax2.plot(t, vel_err, 'orange', label='Velocity Error')
    ax2.set_ylabel('Vel Error (m/s)')
    ax2.grid(True, alpha=0.3)

    # 绘制 p11 参数演化
    ax3.plot(t, p11_history, 'b-', label='p11 (Low Model Prob)')
    ax3.set_ylabel('Parameter p11')
    ax3.set_xlabel('Time (s)')
    ax3.set_ylim(0.5, 1.0)
    ax3.set_title('自适应参数演化 (p11)')
    ax3.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('Ultimate_Adaptive_Result.png', dpi=300)
    plt.show()
